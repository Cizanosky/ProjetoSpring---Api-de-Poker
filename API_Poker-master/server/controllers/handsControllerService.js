...
var convertirValores = function(cartas) {
  ...
  for (var i = 0; i < cartas.length; i++) {
    ...
    if (cartas[i].valor == "2") {
      ...
      v = 10;
    } else if (cartas[i].valor == "Jack") {
      v = 11;
    } else if (cartas[i].valor == "Queen") {
      v = 12;
    } else if (cartas[i].valor == "King") {
      v = 13;
    } else {
      v = 14;
    }
  ...
  try {
    ...
    for (var i = 0; i < req.body.length; i++) {
      ...
      if (req.body[i].jugadas.length != 2) {
        personalizado = true;
        throw new Error( "Cada jogo deve ser composto por exatamente 2 jogadas." );
      }
      ...
      for (var j = 0; j < req.body[i].jugadas.length; j++) {
        ...
        if (req.body[i].jugadas[j].cartas.length != 5) {
          personalizado = true;
          throw new Error( "Cada jogador deve ter exatamente 5 cartas." );
        } else {
        ...
        for (var k = 0; k < req.body[i].jugadas[j].cartas.length; k++) {
          ...
          if (!(/(\W|^)(Clubs|Diamonds|Hearts|Spades)(\W|$)/.test(carta.palo) && /(\W|^)(2|3|4|5|6|7|8|9|10|Jack|Queen|King|Ace)(\W|$)/.test(carta.valor))) {
            personalizado = true;
            throw new Error( "Valor errado em uma carta: {value: " + card.value + ", suit: " + card.suit + " }" );
          }
      ...
      if(amañada) {
        ...
        if(c1 > c2) {
          response.push(j1 + " ganhou");
        } else if(c2 > c1) {
          response.push(j2 + " ganhou");
